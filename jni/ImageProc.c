...
jint 
Java_ch_nuicell_ecam_CameraPreview_prepareCameraWithBase( JNIEnv* env,jobject thiz, jint videoid, jint videobase){
	
		int ret;

		camerabase = videobase;
	
		return Java_ch_nuicell_ecam_CameraPreview_prepareCamera(env,thiz,videoid);
	
}

void 
Java_ch_nuicell_ecam_CameraPreview_processCamera( JNIEnv* env,
										jobject thiz){
...
jint 
Java_ch_nuicell_ecam_CameraPreview_setManualFocus( JNIEnv* env,jobject thiz, jint value){

	int ret;
	SetManualFocusPosition(value);
	return ret;
}

jint 
Java_ch_nuicell_ecam_CameraPreview_setExposure( JNIEnv* env,jobject thiz, jint value){
	...
	if (-1 == ioctl (fd, VIDIOC_QUERYCTRL, &queryctrl)) {
		if (errno != EINVAL) {
			...
			LOGE("V4L2_CID_EXPOSURE_ABSOLUTE is not supported\n");
		}
...
jint 
Java_ch_nuicell_ecam_CameraPreview_setGain( JNIEnv* env,jobject thiz, jint value){
	...
	if (-1 == ioctl (fd, VIDIOC_QUERYCTRL, &queryctrl)) {
		if (errno != EINVAL) {
			...
			LOGE("V4L2_CID_GAIN is not supported\n");
		}
	} else if (queryctrl.flags & V4L2_CTRL_FLAG_DISABLED) {
		LOGE("V4L2_CID_GAIN is not supported\n");
	} else {
...
jint 
Java_ch_nuicell_ecam_CameraPreview_setBrightness( JNIEnv* env,jobject thiz, jint value){
